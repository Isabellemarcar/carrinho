...
const produtos = [
    ...
    // Está criando um objeto chamado container que chama o id container da pagina HTML.
    ...
    // Está criando um objeto chamado carrinhoItens.
    const carrinhoItens = {}

    // Está criando uma função chamada renderizaCarrinho().
    ...
    function criaItemCarrinho(produto) {
        ...
        // Retorna uma string que contém HTML.
        // Ela esta criando uma div com uma classe chamada carrinho_compra.
        // Ela esta inserindo o nome do produto com a tag h4.
        // Ela esta inserindo o preço da unidade e a quantidade.
        // Ela esta fazendo a multiplicação do valor pela quantidade.
        // Ela esta criando um botão que permite remover este item do carrinho. 
        return `
    ...
    // Ela está criando uma função chamada criaCarrinhoTotal.
    function criaCarrinhoTotal() {

        // Ela está criando uma variavel local de valor 0.
        let total = 0;

        //Ela está Iniciando um loop for in com a variavel produtoId que percorre as propriedades do produto carrinhoItens.
        for (let produtoId in carrinhoItens) {

            // Esta parte do codigo calcula o valor total de cada produto no carrinho, multiplicando o preço da unidade do produto (preco_por) pela quantidade desse produto no carrinho (quantidade). O resultado desta conta é adicionado ao valor total na variável total.
            total = total + carrinhoItens[produtoId].preco_por *carrinhoItens[produtoId].quantidade;
        }

        // Esta parte do codigo seleciona o elemento HTML com a classe "carrinho_total" usando o document.querySelector(). Em seguida, atualiza o innerHTML com uma string de HTML.
        // Ela está inserindo o valor total dos produtos ao carrinho.
        // Ela está criando um link com um href vazio e abre em uma nova aba.
        // Ela está inserindo um icone de cartão de crédito.
    ...
    // Ela está criando uma função chamada adicionaItemNoCarrinho chamando o obejeto produto.
    function adicionaItemNoCarrinho(produto) {

        // Ela está verificando se o produto ainda não existe no carrinho, está verificando se a propriedade produto.id não está presente no objeto carrinhoItens.
        // Se o produto ainda não estiver no carrinho, o código dentro do bloco do if é executado.
        if (!carrinhoItens[produto.id]) {

            // Esta parte do codigo adiciona o produto ao carrinho de compras
            carrinhoItens[produto.id] = produto;

            // Esta parte do codigo inicializa a propriedade quantidade para 0 no objeto do produto no carrinho. Isso mostra a quantidade do produto no carrinho.
            carrinhoItens[produto.id].quantidade = 0;

        // Esta parte do codigo aumenta de 1 em 1 a quantidade do produto no carrinho.    
        }++carrinhoItens[produto.id].quantidade;

        // Esta chamando a função renderizaCarrinho() para atualizar os itens no carrinho na página.
        renderizaCarrinho();

        // Esta chamando a função criaCarrinhoTotal() para atualizar o valor total na página.
        criaCarrinhoTotal();}

    // Isto mostra um ouvinte de evento de clique, quando o documento é clicado, a função de é executada.
    document.body.addEventListener('click', function (event) {

        // Tem o elemento que foi clicado e o armazena na variável elemento. O event.target é o elemento que ocorreu o clique.
        const elemento = event.target;

        // Ve se o elemento clicado possui a classe "btn-add", usando o método classList.contains(). Se o elemento tiver essa classe, significa que o botão "Adicionar ao Carrinho" foi clicado.
        if (elemento.classList.contains('btn-add')) {
            ...
            // Essa parte do codigo acessa o objeto do produto no array produtos e o armazena na variável produto.
            const produto = produtos[index];

            // Esta função adicionaItemNoCarrinho() chama os valores do objeto produto, e adiciona esse produto ao carrinho de compras.
            adicionaItemNoCarrinho(produto);
        }

        // Ve se o elemento clicado possui a classe CSS "btn-remove". Se tiver essa classe o loop sera executado.
        if (elemento.classList.contains('btn-remove')){

            // Tem o valor do atributo data-produto-id do botão para saber qual produto deve ser removido do carrinho.
            const produtoId = elemento.getAttribute('data-produto-id');
            
            // Ve se a quantidade desse produto no carrinho é menor ou igual a 1. Se for, o produto é removido do carrinho usando delete carrinhoItens[produtoId]. Caso contrário, a quantidade é decrementada em 1.
            ...
            // por fim parte de codigo chama a propriedade renderizaCarrinho() para atualizar a exibição dos itens do carrinho.
            renderizaCarrinho();

            // Esta parte de codigo chama a propriedade criaCarrinhoTotal() para recalcular e atualizar o valor total do carrinho.
            criaCarrinhoTotal();
        }
    });
